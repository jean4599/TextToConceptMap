...
const parse = (str, delimitre) => {
  ...
  if (str[0] === delimitre) throw new Error("First line can't include tab");
  ...
  if (!rootElement) throw new Error("First line can't include tab");
  ...
  while (nodes.length) {
    ...
    if (curenNode) {
      if (curenNode.level > prevlevel + 1) throw new Error("can't find where to connect this concept\n(too many tabs?)");
